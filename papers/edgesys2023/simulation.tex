...
that is publicly available\footnote{https://github.com/Azzaare/KuMo.jl}.
...
\new{
  The first scenario, depicted in Figure~\ref{fig:edge}, involves a group of drones that move cyclically through a network of interconnected nodes. The drones move through the cycle, gradually transitioning from one node to the next. The total load is kept constant ($1.0$) with interactive jobs.
The different cost models are used for the nodes while the same monotonic cost is used for the links.
With the constant costs, each drone is assigned to the nearest node. In the other cases, the load of each node does not go up beyond $.75$, with the difference in the assignment of the remaining load.
The monotonic costs try to make the remaining load equally shared, while the convex costs try to shift the remaining load to one node. At time $20$ in (c), the remaining load starts to shift from node~1 to node~3 and, once it hits the balancing point, all the remaining load moves to node~3.
This behavior leads to the stability with convex costs in case of turbulence.
%In the convex case, the drones exhibit longer stability phases compared to the monotonic case, with fewer active resources (nodes and links) being utilized at any given time. Another significant difference is observed at the end of a cycle, such as at time $20$, when the drones begin to transition, leading to a shift in the extra loads.
}

% This is a comment
% This code is used to plot the results of the simulation
% The results are stored in the "results" variable
% The variable "results" is a 4x4 matrix
% The first row is for the constant behavior
% The second row is for the convex behavior
% The third row is for the monotonic behavior
% The fourth row is for the linear behavior
% The first column is for the average cost
% The second column is for the average number of collisions
% The third column is for the average number of vehicles
% The fourth column is for the average number of drones
% The first row is for the scenario with 10 vehicles and 25 drones
% The second row is for the scenario with 20 vehicles and 50 drones
% The third row is for the scenario with 30 vehicles and 75 drones
% The fourth row is for the scenario with 40 vehicles and 100 drones
% The following code plots the average number of drones
% plot(results(:,4))

\begin{figure}[tb]
  \begin{center}
    \includegraphics[width=1.0\columnwidth]{placeholder.pdf}
    \vspace{-4.0ex}
