...
\new{
  The first scenario, depicted in Figure~\ref{fig:edge}, involves a group of drones that move cyclically through a network of interconnected nodes or routers. The drones move smoothly and uniformly through the cycle, gradually transitioning from one node to the next. They periodically request interactive jobs as they traverse the network.

  This scenario highlights the differences between the monotonic and convex cases, with the behavior for constant pseudo-costs presented as a reference. In the convex case, the drones exhibit longer stability phases compared to the monotonic case, with fewer active resources (nodes and links) being utilized at any given time. Another significant difference is observed at the end of a cycle, such as at time $20$, when the drones begin to transition, leading to a shift in the extra loads.
...
\begin{figure}[tb]
  \begin{center}
    ...
    \vspace{-4.0ex}
    \caption{Comparing (a) constant, (b) convex and (c) monotonic behaviors with
