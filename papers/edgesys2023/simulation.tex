...
\new{
  In the first scenario, Figure~\ref{fig:edge}, a group of drones moves in a cyclic pattern through a series of interconnected nodes or routers. The drones move seamlessly and uniformly through the 20-unit cycle, gradually transitioning from one node to the next. This scenario showcases the longer stability phases in the convex case with fewer active resources (nodes and links) at a time. The other significant difference is observed at the end of a cycle when the drones begin transitioning, causing a shift in the extra loads.
}
...
\begin{figure}[tb]
  \begin{center}
    ...
    \vspace{-4.0ex}
    \caption{Comparing (a) baseline, (b) convex and (c) monotonic behaviors with
