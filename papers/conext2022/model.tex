...
One can {\bf lower or raise the utilization} of a resource by shifting
the load and adjusting the target load, $f'(\rho) = f(\rho + \Delta)$.
...
in the same way as lowering the utilization
but for specific users or jobs (not for a specific resource) so as to
have premium jobs being assigned to less loaded resources.
Similarly, an {\bf economy service} that allows to be assigned to more
loaded resources can be made by shifting the load in the opposite direction.
...
weights.  Also, it is effective to place data objects close to the
