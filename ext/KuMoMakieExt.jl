...
function show_pseudo_costs(pseudo_costs, ::Val{:interactive})
    ...
    sg = SliderGrid(fig[2, 1],
        ...
        (label = "Multiplicative", range = 0:0.1:10,
            startvalue = 1, format = "cost × {:.1f}"),
        (label = "Load shift", range = 0:0.01:0.5,
            startvalue = 0, format = "load + {:.01f}"),
...
function show_simulation(df, norms, _, ::Val{:interactive})
    ...
    m1 = Menu(fig[3, :][1, 4], prompt = "Select plot type(s)...",
        options = ["all", "resource load", "total load"], width = 300)
    m2 = Menu(fig[3, :][1, 3], prompt = "Select output format...",
        options = [".png"], width = 300)
...
function show_simulation(df, norms, select, ::Val{:static})
    ...
    for c in a:b
        γ = c - a + 1
        selected(c) &&
            lines!(df[!, :instant], df[!, c]; color = colors[γ], label = labels[c])
    end
    ...
    for c in 1:length(a:b)
        γ = a - 1 + c
        selected(γ) && band!(
            df[!, :instant], Y[:, c], Y[:, c + 1], color = colors[c], label = labels[γ])
    end
    ...
    if d !== nothing
        ...
        for c in α:δ
            γ = c - b
            selected(c) && band!(df[!, :instant], Y[:, γ], Y[:, γ + 1],
                color = colors[c - a + 1], label = labels[c])
        end
...
function KuMo.show_simulation(
        df::DataFrame, norms = Dict(); interaction = :auto, select = nothing)
    cdf = KuMo.clean!(df)
    cb = string(Makie.current_backend())
    is_static = occursin(CAIROMAKIE, cb) || occursin(RPRMAKIE, cb)
    if interaction ∈ [:static] || is_static
...
function KuMo.show_simulation(
        path::String; norms = Dict(), interaction = :auto, select = nothing)
    df = DataFrame(CSV.File(path; delim = ','))
    return KuMo.show_simulation(df, norms; interaction, select)
end

function KuMo.show_simulation(
        s::Symbol = :_four_nodes; norms = Dict(), interaction = :auto, select = nothing)
...
function KuMo.show_interactive_run(;
    ...
    @async begin
        while !isready(agent.containers.stop)
            ...
            for c in a:δ
                γ = c - a + 1
                _lines[γ] = lines!(results[!, :instant], results[!, c];
                    color = colors[γ], label = labels2[c])
                connect!(_lines[γ].visible, toggles[γ].active)
            end
