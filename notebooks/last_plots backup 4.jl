...
begin
	...
	df12 = DataFrame()
	df12.instant = df10[!,:instant]
	df12.ratio = sum.(eachrow(df10[:,6:23])) ./ sum.(eachrow(df10[:,24:end]))

	df13 = DataFrame()
	df13.instant = df10_no_norm[!,:instant]
	df13.ratio = sum.(eachrow(df10_no_norm[:,6:23])) ./ sum.(eachrow(df10_no_norm[:,24:end]))
end;
