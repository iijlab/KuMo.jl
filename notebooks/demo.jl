...
begin 
	...
	!!! tip "Basic use"
		...
		Note that there is an additional step at the current time as the package is not yet registered on the General registry.
	
		```julia
		] add https://github.com/iijlab/KuMo.jl
		```
	
		In the Julia console (REPL), using KuMo with a graphical interface is an simple as running the following line:
...
# ╔═╡ e8a5df10-0ea5-43ba-97e5-45497999db5d
agent = show_interactive_run(); sleep(5)

# ╔═╡ 78c80be4-d041-40ea-9c47-f021d5fccbb4
md"""
## Adding infrastructure and resources
!!! info "Playing with nodes"
	By default nodes are built with a convex pseudocost function that represents a natural variation of the cost of server nodes.

	```julia
	node!(agent, node_resource) # adding new node with `node_id` next unused id 
	node!(agent, node_id) # remove node
	node!(agent, node_id, my_node_resource) # replace the pseudo-cost function
	```

	The network infrastructure and the nature of the resources in it can be changed dynamically.


Let's start by adding some nodes with capacities of 50 containers.

Instead of repeating the command above `n` times, we could use the following iteration:

```julia
foreach(_ -> node!(agent, KuMo.Node(50)), 1:n)
```
"""

# ╔═╡ f0957620-92dd-4441-90e0-62909c071f96
# Adding 8 nodes
foreach(_ -> node!(agent, KuMo.Node(50)), 1:8)

# ╔═╡ 5746108a-fac2-4d79-be66-778b4492db9b
md"""
!!! info "Linking the nodes"
	By default, links costs are defined as a monotonic increasing function.
	```julia
	...
	One can also use free links (constant or no costs)

	```julia
	link!(agent, source, target, KuMo.FreeLink())
	```

	For the sake of visualization, we will use `FreeLink`s here.

We should also connect those nodes together. We choose a group of 2 clusters of 4 nodes connected twice.
"""
...
begin
	...
	link!(agent, 5, 6, KuMo.FreeLink())
	link!(agent, 6, 7, KuMo.FreeLink())
	link!(agent, 7, 8, KuMo.FreeLink())
	link!(agent, 8, 5, KuMo.FreeLink())

	link!(agent, 3, 5, KuMo.FreeLink())
	link!(agent, 6, 4, KuMo.FreeLink())
end;
...
# job!(agent, backend, container, duration, frontend, data_id, user_id, ν; stop = Inf)
job!(agent, 0, 2, 2, 0, 2, 2, 0.01);

# ╔═╡ aaa97db5-3a6d-4ceb-a8f3-85425e93649a
sleep(2); job!(agent, 0, 1, 1, 0, 1, 1, 0.01; stop=5.0);

# ╔═╡ 19e54237-70ff-4548-8b3c-0930a989c0c2
# job!(agent, backend, container, duration, frontend, data_id, user_id, ν; stop = Inf)
begin
	jmax = 30
	for j in 1:jmax
		sleep(3.5+rand())
		job!(agent, 0, j % 5 + 1, 1, 0, rand(1:2), rand(1:2), 0.01; stop= 2jmax - 2.0 * j)
	end
end;
...
# ╔═╡ 1b846373-b047-41df-83ce-917aa935eb6d
# ╠═╡ disabled = true
#=╠═╡
show_pseudo_costs()
  ╠═╡ =#

# ╔═╡ b68bfd03-7689-40be-a030-2096965050d9
node!(agent, KuMo.IdleStateNode(200, 5));

# ╔═╡ 2712b376-845e-41f3-8b17-52f137ceffa5
md"""
We can make or change the different resources with the following commands.

!!! tip "Modifying node resources"
	```julia
...
!!! tip "Convex pseudo-cost (default)"
...
!!! info "Convex pseudo-cost: Additive"
...
!!! info "Convex pseudo-cost: Multiplicative"
...
!!! info "Convex pseudo-cost: IdleState"
...
!!! info "Convex pseudo-cost: Premium"
...
!!! info "Monotonic pseudo-cost: EqualLoadBalancing"
...
!!! info "Constant pseudo-cost: Flat"
...
!!! tip "Modifying link resources"
	```julia
	link!(agent, source, target, link_resource) # adding/change new/existing link
	link!(agent, source, target) # remove link
	```

!!! tip "Monotonic pseudo-cost: Link (default)"
...
!!! info "Convex pseudo-cost: Convex"
...
manifest_format = "2.0"
project_hash = "2ac3963215af562ecc97c8347f891bdca3342754"
...
[[deps.ArrayTools]]
deps = ["TypeUtils"]
git-tree-sha1 = "c198747399db18feefa67cb1485cf4c5f6489b78"
...
[[deps.KuMo]]
deps = ["CSV", "DataFrames", "DataStructures", "Dictionaries", "Distributions", "DrWatson", "Graphs", "PrettyTables", "ProgressMeter", "Random", "ResizableArrays", "SparseArrays", "TestItems"]
git-tree-sha1 = "3474d539e6fadba18bca4c71ee75076ee7d993aa"
uuid = "b681f84e-bd48-4deb-8595-d3e0ff1e4a55"
version = "0.2.1"

    [deps.KuMo.extensions]
    KuMoMakieExt = ["Colors", "Makie"]
    KuMoPlotsExt = ["StatsPlots"]
...
[[deps.ResizableArrays]]
deps = ["ArrayTools"]
git-tree-sha1 = "e6dcf5f2dc5ec222474fb51788757c7484956241"
uuid = "df514bf8-98b6-11e9-19f1-af1e340ea726"
version = "0.3.5"
...
[[deps.TypeUtils]]
deps = ["Requires"]
git-tree-sha1 = "720c991d0123f943488df0bc8f0c99e41cee13a7"
